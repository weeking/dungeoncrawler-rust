...
pub fn chasing(#[resource] map: &Map, ecs: &SubWorld, commands: &mut CommandBuffer) {
    ...
    let player_ind = map_ind(player_pos.x, player_pos.y);
    let search_targets = vec![player_ind];
    ...
    movers.iter(ecs).for_each(|(entity, pos, _, fov)| {
        ...
        let ind = map_ind(pos.x, pos.y);
        if let Some(destination) = DijkstraMap::find_lowest_exit(&dijkstra_map, ind, map) {
